...
function partion(array, left, right) {
  ...
  for (let j = left; j <= right - 1; j += 1) {
    if (array[j] < pivot) {
      i += 1;
      [sortedArray[i], sortedArray[j]] = [array[j], array[i]];
    }
  }
  [sortedArray[i + 1], sortedArray[right]] = [array[right], array[i + 1]];
  return i + 1;
}

function sortFn(array, l, r) {
  if (l < r) {
    const m = partion(array, l, r);
    sortFn(array, l, m - 1);
    sortFn(array, m + 1, r);
  }
}
function sortByAsc(arr) {
  return sortFn(arr, 0, arr.length - 1);
}
...
function getNearestBigger(number) {
  ...
  const leftArr = [];
  let rightArr = [];
  const arr = Array.from(`${number}`).map(Number);
  ...
  for (let k = i; k < arr.length; k += 1) {
    rightArr = [...rightArr, arr[k]];
  }
  return Number([...leftArr, ...rightArr.reverse()].join(''));
}
getNearestBigger(90822);
module.exports = {
  isPositive,
  getMaxNumber,
  canQueenCaptureKing,
